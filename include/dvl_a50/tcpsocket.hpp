...
public:
    ...
    void Receive(char *tempBuffer)
    {
        int messageLength = recv(this->sock, tempBuffer, buffer_size, 0);
...
private:
	...
	int iErr;
	iErr = setsockopt(this->sock, SOL_SOCKET, SO_RCVTIMEO, (char*) &timeout, sizeof(struct timeval));
	if (iErr < 0) {
            close(sock);
	    return false;
	}
